...
if [[ "$DRY_RUN" == true ]]; then
  ...
  printf '  existing %s and %s -> preserved\n' "$AGENTS_TARGET" "$CONFIG_TARGET"
  for skill in "${BASE_SKILLS[@]}"; do
...
if [[ -f "$AGENTS_TARGET" ]]; then
  cp -p "$AGENTS_TARGET" "$PRESERVE_DIR/AGENTS.md"
  PRESERVE_AGENTS=true
fi

if [[ -f "$CONFIG_TARGET" ]]; then
  cp -p "$CONFIG_TARGET" "$PRESERVE_DIR/config.toml"
...
restore_codex_user_files() {
  if [[ "$PRESERVE_AGENTS" == true ]]; then
    mkdir -p "$CODEX_ROOT"
    cp -p "$PRESERVE_DIR/AGENTS.md" "$AGENTS_TARGET"
  fi
  if [[ "$PRESERVE_CONFIG" == true ]]; then
    mkdir -p "$CODEX_ROOT"
    cp -p "$PRESERVE_DIR/config.toml" "$CONFIG_TARGET"
  fi
}
...
COMPILED_RULES="$(mktemp "${TMPDIR:-/tmp}/mv-codex-global-rules.XXXXXX")"
mv_compile_markdown_rules \
  "$COMPILED_RULES" \
  "MV-Platform Global Rules" \
  "$RULES_TARGET" \
  "$(dirname "$GLOBAL_RULE_TARGET")"
mv_replace_markdown_block "$AGENTS_TARGET" "ecc-global-rules" "$COMPILED_RULES"
rm -f "$COMPILED_RULES"
